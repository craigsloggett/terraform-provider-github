...
- `merge_commit_message` (String) The default value for a merge commit message. `PR_TITLE` defaults to the pull request's title, `PR_BODY` defaults to the pull request's body, and `BLANK` defaults to a blank commit message. Can be one of: `PR_BODY`, `PR_TITLE`, `BLANK`.
